...
export default function Admin() {
  ...
  async function createCompany(e) {
    ...
    try {
      ...
      if (!data.success) {
        setError(
          data.error === "USERNAME_ALREADY_EXISTS"
  ...
  async function deleteCompany(id, nome) {
    ...
    const conferma = confirm(
      `Eliminare l'azienda "${nome}" e tutti i suoi dati?`
    );
    ...
    try {
      ...
      const response = await fetch(
        API_URL + "/api/admin/companies/" + id,
        {
          method: "DELETE",
          headers: { Authorization: "Bearer " + token }
        }
  ...
  async function addFascia(companyId) {
    ...
    try {
      ...
      const response = await fetch(
        API_URL + "/api/admin/companies/" + companyId + "/fasce",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!data.success) {
        alert("Errore salvataggio fascia");
        return;
      }

      setShowFasciaForm(null);
      setFasciaNome("");
      setFasciaOraInizio("07:00");
      setFasciaOraFine("10:00");
      setFasciaTipo("ENTRATA");
      loadCompanies();
  ...
  async function deleteFascia(fasciaId) {
    ...
    const conferma = confirm("Eliminare questa fascia oraria?");
    if (!conferma) return;
    ...
    try {
      ...
      const response = await fetch(
        API_URL + "/api/admin/fasce/" + fasciaId,
        {
          method: "DELETE",
          headers: { Authorization: "Bearer " + token }
        }
      ...
      if (!data.success) {
        alert("Errore eliminazione fascia");
        return;
      }
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {showCreate && (
          ...
          <div className="rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#161618] p-6 mb-8">
            ...
            <form onSubmit={createCompany} className="grid md:grid-cols-3 gap-4">
              ...
              {error && (
                <p className="md:col-span-3 text-sm text-red-500">{error}</p>
              )}

              <button
                type="submit"
                disabled={saving}
                className="md:col-span-3 h-12 rounded-2xl bg-zinc-900 text-white dark:bg-zinc-100 dark:text-black font-medium disabled:opacity-50"
              >
                {saving ? "Creazione..." : "Crea azienda"}
              </button>
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-5">
            ...
            {companies.map((company) => (
              ...
              <div
                key={company.id}
                className="rounded-3xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-[#161618] overflow-hidden"
              >
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => deleteCompany(company.id, company.nome)}
                  ...
                  <div className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl px-4 py-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-mono text-zinc-700 dark:text-zinc-300 truncate max-w-[260px]">
                        {company.id}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        ...
                        {company.users.map((user) => (
                          ...
                          <div
                            key={user.id}
                            className="rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4"
                          >
                            ...
                            {changingPassword === user.id && (
                              ...
                              <div className="mt-3 flex gap-2">
                                ...
                                <button
                                  ...
                                  className="h-9 px-4 rounded-xl bg-zinc-900 dark:bg-zinc-100 text-white dark:text-black text-sm font-medium"
                                >
                                  Salva
                                </button>
                                ...
                                <button
                                  ...
                                  className="h-9 px-3 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 text-sm"
                                >
                                  Annulla
                                </button>
                        ...
                        <div className="mb-3 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4">
                          ...
                          <div className="grid grid-cols-2 gap-3 mb-3">
                            ...
                            <div>
                              ...
                              <input
                                type="time"
                                value={fasciaOraInizio}
                                onChange={(e) => setFasciaOraInizio(e.target.value)}
                            ...
                            <div>
                              ...
                              <input
                                type="time"
                                value={fasciaOraFine}
                                onChange={(e) => setFasciaOraFine(e.target.value)}
                                className="w-full h-10 px-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 outline-none"
                              />
                            </div>
                          ...
                          <div className="mb-3">
                            ...
                            <input
                              type="text"
                              placeholder={`es. ${fasciaTipo === "ENTRATA" ? "Entrata mattina" : "Uscita pomeriggio"}`}
                              value={fasciaNome}
                              onChange={(e) => setFasciaNome(e.target.value)}
                              className="w-full h-10 px-3 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-sm text-zinc-900 dark:text-zinc-100 outline-none"
                            />
                          </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => setShowFasciaForm(null)}
                      ...
                      {company.fasce && company.fasce.length > 0 ? (
                        ...
                        <div className="space-y-2">
                          ...
                          {company.fasce.map((fascia) => (
                            ...
                            <div
                              key={fascia.id}
                              className="flex items-center justify-between rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-4 py-3"
                            >
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <button
                                  onClick={() => deleteFascia(fascia.id)}
                        ...
                        <div className="rounded-2xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-4 py-6 text-center">
                          ...
                          <p className="text-xs text-zinc-400 mt-1">
                            Senza fasce la logica usa l'ultima timbratura
                          </p>
                        </div>
